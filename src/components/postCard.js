...
export default (props) => (
	<article
		className={`post-card ${props.count % 3 === 1 && `post-card-large`} ${props.postClass} ${props.node.frontmatter
			.thumbnail
			? `with-image`
			: `no-image`}`}
		style={
	...
	render() {
		...
		return (
			<div className="post-card-content">
				...
				<div className="post-card-body">{props.node.frontmatter.description || props.node.excerpt}</div>
				<div>
					<Link to={props.node.fields.slug} className="post-card-link post-card-readmore">
						{props.node.frontmatter.description || props.node.excerpt ? 'Ler mais' : null}
					</Link>
				</div>
			</div>
		);
